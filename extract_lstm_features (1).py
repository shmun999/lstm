...
# ===== 여러 경기 일괄 처리 =====
def extract_multiple_matches(match_ids: list[str], output_path: str = "lstm_features.csv"):
    all_rows = []
    ...
    output_path = os.path.join("csv", output_path)
    ...
    print(f"\n✅ 저장 완료: {output_path}")
    print(f"   총 {len(all_rows)}행 × {len(fieldnames)}열")
    print(f"   경기 수: {len(match_ids)}개")
    print(f"\n📋 피처 목록 ({len(fieldnames)}개):")
...
if __name__ == "__main__":
    ...
    OUTPUT_FILE = "lstm_features.csv"

    extract_multiple_matches(MATCH_IDS, OUTPUT_FILE)
