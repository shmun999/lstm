...
# ===== API 호출 =====
# 개발용 키 제한: 20요청/1초, 100요청/2분
# 경기 1개당 API 2회 호출 (match + timeline)
# → 100요청 / 2 = 50경기 / 2분 → 경기당 2.4초 대기
API_DELAY = 1.5   # 요청 사이 대기 시간 (초) — 429 에러 시 늘려주세요
...
def api_get(url: str) -> dict | None:
    while True:
        ...
        if response.status_code == 200:
            ...
            # 요청 한도 초과 → Retry-After 헤더 확인 후 대기
            retry_after = int(response.headers.get("Retry-After", 10))
            print(f"  ⏳ 요청 한도 초과 (429) — {retry_after}초 대기 중...")
...
# ===== CSV 파일에서 경기 ID 읽기 =====
def load_match_ids_from_csvs(csv_dir: str = "match_ids") -> list[str]:
    import glob
    files = glob.glob(os.path.join(csv_dir, "*_match_ids.csv"))
    if not files:
        raise FileNotFoundError(f"'{csv_dir}' 폴더에 match_ids CSV 파일이 없습니다.")

    all_ids = []
    for file in files:
        with open(file, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            ids = [row["match_id"] for row in reader if row.get("match_id")]
            all_ids.extend(ids)
        print(f"  📄 {os.path.basename(file)}: {len(ids)}개 로드")

    unique_ids = list(dict.fromkeys(all_ids))
    print(f"\n  총 {len(all_ids)}개 중 중복 제거 후 {len(unique_ids)}개")
    return unique_ids


# ===== 실행 =====
if __name__ == "__main__":
    MATCH_IDS_DIR = "match_ids"

    print(f"📂 '{MATCH_IDS_DIR}' 폴더에서 경기 ID 로드 중...")
    MATCH_IDS = load_match_ids_from_csvs(MATCH_IDS_DIR)

    total     = len(MATCH_IDS)
    # 경기 1개당 API 2회 × 딜레이 = 예상 소요 시간
    eta_sec   = total * 2 * API_DELAY
    eta_min   = eta_sec / 60
    print(f"  ⏱  예상 소요 시간: 약 {eta_min:.0f}분 ({total}경기 × 2회 × {API_DELAY}초)\n")

    success, fail = 0, 0
    for i, match_id in enumerate(MATCH_IDS):
        print(f"[{i+1}/{total}] {match_id} 처리 중...")
